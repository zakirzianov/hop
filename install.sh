...
show_instructions() {
    ...
    echo -e "У вас должны быть данные от конечного сервера (VPN/Прокси и т.д.):"
    echo -e " - ${YELLOW}IP адрес${NC} (конечный сервер)"
    echo -e " - ${YELLOW}Порт${NC} (на котором работает целевой сервис, например: 443 или 51820)"
    echo ""
    echo -e "${CYAN}ШАГ 2: Настройка этого сервера${NC}"
    echo -e "1. Выберите нужный пункт (${GREEN}1-2${NC} для стандартных или ${GREEN}3${NC} для кастомных)."
    echo -e "2. Введите ${YELLOW}IP${NC} и ${YELLOW}Порты${NC} (входящий и исходящий)."
    echo -e "3. Скрипт создаст 'мост' через этот VPS."
    echo ""
    echo -e "${CYAN}ШАГ 3: Настройка Итогового файла конфигурации (Важно!)${NC}"
    echo -e "1. Откройте файла конфигурации конечного vpn сервера (WG/AWG или Vless )."
    ...
    echo -e "4. Если вы использовали разные порты в правиле 3, укажите Входящий порт."
    echo ""
    echo -e "${GREEN}Готово! Теперь трафик идет: Клиент -> Hop Сервер -> Конечный сервер.${NC}"
    echo ""
...
configure_custom_rule() {
    ...
    echo -e "${BLUE}Подходит для перенаправления ЛЮБЫХ протоколов (SSH, RDP, нестандартные порты)."
...
show_menu() {
    while true; do
        ...
        echo -e "3) 🛠 Создать Кастомное правило (Разные порты, SSH, RDP...)"
